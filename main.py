...
# Determine storage directory for PDFs and legacy files
...
def log_event(event: str, username: str = "-", ip: str = "-"):
    ...
    try:
        ...
        print(f"[WARN] Could not write to audit log: {e}", file=sys.stderr)
...
def load_api_keys() -> List[dict]:
    """Load API keys from legacy JSON file (backward compatibility)"""
...
def load_sessions() -> List[dict]:
    """Load sessions from legacy JSON file (backward compatibility)"""
...
def get_or_create_default_shop(db: Session) -> Shop:
    """Get or create default shop for backward compatibility"""
    ...
    if not shop:
        shop = Shop(
            ...
            currency="EUR",
            invoice_prefix="INV",
            api_key_hash=hashlib.sha256(b"default").hexdigest(),
            plan="starter"
        )
        db.add(shop)
        db.commit()
...
def create_user(db: Session, email: str, password_hash: str, role: str, shop_id: str, name: str = None) -> DBUser:
...
def get_invoice_by_id(db: Session, invoice_id: str) -> Optional[DBInvoice]:
    """Get invoice by ID"""
    return db.query(DBInvoice).filter(DBInvoice.id == invoice_id).first()


def get_invoices_by_shop(db: Session, shop_id: str, skip: int = 0, limit: int = 100) -> List[DBInvoice]:
    """Get all invoices for a shop"""
    return db.query(DBInvoice).filter(DBInvoice.shop_id == shop_id).offset(skip).limit(limit).all()


def create_customer(db: Session, shop_id: str, name: str, email: str, country: str, address: dict, vat_number: str = None) -> Customer:
...
def log_event_to_db(db: Session, action: str, shop_id: str = None, actor: str = "-", ip: str = "-", target: str = None, extra_data: dict = None):
    ...
    try:
        ...
        print(f"[WARN] Failed to log event to database: {e}", file=sys.stderr)
...
async def get_current_user(request: Request, db: Session = Depends(get_db)):
    ...
    3. Non-production fallback to the first user for local dev convenience
    """
    ...
    if api_key:
        try:
            ...
            # Check database for Shop by API key
            shop = db.query(Shop).filter(Shop.api_key_hash == key_hash).first()
            if shop:
                # Return shop owner (first admin user)
                user_db = db.query(DBUser).filter(
                    DBUser.shop_id == shop.id,
                    DBUser.role == "admin"
                ).first()
                if user_db:
                    return {
            ...
            # Fallback to file-based API keys for backward compatibility
            keys = load_api_keys()
    ...
    if not IS_PROD:
        ...
        return {"id": "dev", "name": "dev", "email": "dev@localhost", "role": "admin", "shop_id": shop.id, "token_version": 1}
...
async def list_users(
    ...
    """List all users in the current user's shop"""
    shop_id = current_user.get("shop_id")
    users = db.query(DBUser).filter(DBUser.shop_id == shop_id).all()
    return [
        {
            "id": str(u.id),
...
async def get_user(
    ...
    if not user or user.shop_id != current_user.get("shop_id"):
...
async def add_user(
    ...
    # Create new user in the same shop as admin
...
async def register_merchant(
    ...
    country = payload.get("country", "NL").strip().upper()
    ...
    shop = Shop(
        ...
        country=country,
        address={"street": "", "city": "", "zip": "", "country": country},
        currency="EUR",
        invoice_prefix="INV",
    ...
    access_token = create_access_token(
        data={"sub": email, "role": "merchant"}
    )

    log_event_to_db(db, "USER_REGISTERED", shop_id=shop.id, actor=email)
...
async def login_for_access_token(
    ...
    """Login endpoint - supports email or username"""
    ...
    if login.email:
        ...
        # Try as email first, then as name
        user = get_user_by_email(db, login.name)
        if not user:
            user = db.query(DBUser).filter(DBUser.name == login.name).first()
    ...
    valid = False
    try:
        valid = pwd_context.verify(login.password, user.password_hash)
    except Exception:
        valid = False
    ...
    log_event("LOGIN_SUCCESS", user.email, ip)
    log_event_to_db(db, "LOGIN_SUCCESS", shop_id=user.shop_id, actor=user.email, ip=ip)

    # Return canonical auth response
    return {
        "access_token": access_token,
        "token_type": "bearer",
        "merchant_id": str(user.id),
